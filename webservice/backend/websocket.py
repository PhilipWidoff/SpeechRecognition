...
logger.info(
    f"Google Cloud API Key Path: {os.getenv('GOOGLE_CLOUD_API_KEY_PATH')}")
...
async def process_audio(audio, worker_id):
    ...
    try:
        ...
        logger.debug(
            f"Audio file size: {os.path.getsize(temp_audio_path)} bytes")
        ...
        logger.error(
            f"Transcription error on {'GPU' if use_gpu else 'CPU'} {worker_id}: {e}", exc_info=True)
        return None
    finally:
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            if "text" in message:
                ...
                tts_audio = text_to_speech(
                    translated_text, language_code=target_language)

                tts_audio_base64 = base64.b64encode(
                    tts_audio).decode('utf-8') if tts_audio else None
        ...
        logger.error(
            f"Error during WebSocket communication: {e}", exc_info=True)
    finally:
